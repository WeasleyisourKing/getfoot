...
//        $products = $arr;
        ...
        return $this->place($uid, $uProducts, $code);
     ...
     * @return $order 订单号
     */
    public function place ($uid, $uProducts, $code)
    {
...
//        $snapshootOrder['freight'] = ($snapshootOrder['orderPrice'] <= $freight['threshold']) ? $freight['freight'] : 0;
        ...
        //写入数据库 订单号 购买商品
        //订单和商品关系 多对多
        $order = self::createOrder($snapshootOrder, $backup);
...
//        dd($userAddress);
    ...
    public function editAddress (Request $request)
    {
        ...
        $order = OrderModel::where('id', '=', $params['order_id'])->first(['tax', 'total_price', 'freight']);
        ...
        $res = OrderModel::where('id', '=', $params['order_id'])->update($data);
        if ($res) {
            return 1;
        } else {
            return 0;
...
//            dd($pStatus);
    ...
    //因为插入2张表 使用事务
    private function createOrder ($orderSnap, $backup)
    {
        $num = OrderModel::orderBy('created_at', 'desc')->first(['order_no']);
        $orderNo = Common::makeOrderNo($num->order_no);
...
//                    $text .= '商品' . $item['name'] . '库存不足；';
    ...
    public function palNotify (Request $request)
    {
        ...
        $order = OrderModel::getOrderInfo($params['item_number']);
        ...
        if ($order->status == 2) {
            ...
            OrderModel::updateStatus($order->id, ['status' => 1]);
...
//        return $data;
    ...
    public function countOrderIndex (Request $request)
    {
        ...
        $res = OrderModel::select(\Illuminate\Support\Facades\DB::raw('count(id) as count,status'))
            ->where('users_id', '=', $id)
            ->groupBy('status')
            ->get();
