...
@section('content')
  ...
  <div class="container bg-gradient pb-2">
    <div class="d-flex justify-content-between pt-3 align-items-center">
      ...
      <div class="col-4 px-2 d-flex justify-content-around d-block d-sm-none">
        <a href="{{ route('login') }}" class="btn btn-circle btn-white shaddow-light"><i class="fa fa-user"></i></a>
  ...
  <div class="container position-relative">
    ...
    <div class="d-flex flex-row scrolling-wrapper-flexbox rounded shaddow-dark">
      ...
      <div class="col-12 p-0">
        <img class="w-100 h-100" src="{{$banner[0]->img->url}}" alt="">
      </div>
      @endfor
  ...
  <div class="container mt-3">
    <div class="d-flex flex-row justify-content-between">
      ...
      <div>
        ...
        <small class="d-block py-2">休闲零食</small>
      </div>
      @endfor
    </div>
  ...
  <div class="container mb-2 pb-1">
    ...
    <div class="row justify-content-center">
      ...
      <div class="card shaddow-dark mb-2 mx-1 w-30 border-0">
        <a href=""><img src="{{$modular[0]['products'][$i]['product_image']}}" class="img-fluid card-img-top rounded-top img-bg-{{$i+1}} mb-1" alt=""></a>
        <div class="card-body p-1">
          <small class="mt-0 d-block text-dark">{{$modular[0]['products'][$i]['zn_name']}}</small>
          <h6 class="card-title d-inline text-red">${{$modular[0]['products'][$i]['distributor']['level_four_price']}}</h6>
          <a class="card-link btn btn-sm bg-color-red text-white rounded-50 py-0"product_image="{{$modular[0]['products'][$i]['product_image']}}"zn_name="{{$modular[0]['products'][$i]['zn_name']}}"
          price="{{$modular[0]['products'][$i]['distributor']['level_four_price']}}"shop_id="{{$modular[0]['products'][$i]['id']}}" onclick="joinShopCart()">购买</a>
        </div>
      </div>
      @endfor
  ...
  <div class="container mb-2 px-0">
    ...
    <div class="container">
      <div class="d-flex justify-content-between mt-4">
        <div><h4 class="pl-2 title-border">热销商品</h4></div>
      </div>
    </div>
    ...
    <div class="d-flex flex-row scrolling-wrapper-flexbox">
      ...
      <div class="card bg-color-{{$i+1}} border-0 mb-2 mx-1 w-25 mw-25 mt-5">
        <a href=""><img src="{{$modular[1]['products'][$i]['product_image']}}" class="img-fluid card-img-top rounded img-bg-{{$i+1}} mb-1 position-relative hot-sale-img shaddow" alt=""></a>
        <div class="card-body hot-sale-card-body text-white text-center p-1">
          <small class="mt-0 d-block">{{$modular[1]['products'][$i]['zn_name']}}</small>
          <h6 class="card-title mb-0">${{$modular[1]['products'][$i]['distributor']['level_four_price']}}</h6>
          <a class="card-link btn btn-sm bg-light text-dark rounded-50 opactity-50 mt-0 py-0 px-3"product_image="{{$modular[1]['products'][$i]['product_image']}}"zn_name="{{$modular[1]['products'][$i]['zn_name']}}"
          price="{{$modular[1]['products'][$i]['distributor']['level_four_price']}}"shop_id="{{$modular[1]['products'][$i]['id']}}">购买</a>
        </div>
      </div>
      @endfor
  ...
  <div class="container mb-2 pb-1">
    ...
    <div class="d-flex justify-content-between my-4">
      <div><h4 class="pl-2 title-border">分类商品</h4></div>
      <div><a href="">更多 <i class="fa fa-angle-right"></i></a></div>
    </div>
    ...
    <div class="row justify-content-center">
      ...
      @for( $i = 0; $i < 6; $i++ )
      @if (!empty($categorys[$i]))
      
      <div class="card shaddow mb-2 mx-1 border-0 w-30">
        <a href=""><img src="" class="img-fluid card-img-top rounded-top mb-1" alt=""></a>
        <div class="card-body p-1">
          <small class="mt-0 d-block text-dark">商品{{$i+1}}名称</small>
          <h6 class="card-title d-inline text-red">$33.99</h6>
          <a class="card-link btn btn-sm bg-color-red text-white rounded-50 py-0">购买</a>
        </div>
      </div>
      @endif
      @endfor
